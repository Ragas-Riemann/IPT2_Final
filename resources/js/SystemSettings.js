...
export default function SystemSettings({ embed = false }){
  ...
  async function handleDepartmentDelete(id){
    if(!window.confirm('Delete this department? This will also remove all associated courses.')) return;
    try{ 
  ...
  const coursesView = (
    React.createElement(React.Fragment, null,
      React.createElement('div', { className: 'card' },
        React.createElement('div', { className: 'card-header', style: { display: 'flex', alignItems: 'center', gap: '16px' } },
          React.createElement('button', {
          ...
          React.createElement('h2', { style: { margin: 0, flex: 1 } }, 
